export const CardCourse = ({ img, title, content, children, footer = true, nav, subTitle }) => {

  const altImg = `https://ui-avatars.com/api?background=random&name=${title}`

  return (
      <article className="bg-white shadow rounded border-2 border-blue-300 group w-72
          ...
          < div className="relative overflow-hidden cursor-pointer">
              ...
              <div className="h-96 w-72">
                  <img src={img ? img : altImg} alt={`${title} img`}
                      className="h-full w-full object-cover duration-500 transition-transform
                  group-hover:scale-125" />
              </div>

              {/* OSCURIDAD POR ENCIMA*/}
              <div className="absolute inset-0 bg-gradient-to-b from-transparent via-transparent to-black/5 group-hover:from-black/60 group-hover:via-black/50 group-hover:to-black/60"></div>

              {/* TITULO Y CONTENIDO */}
              <div
                  onClick={nav}
                  className="p-5 space-y-3 absolute inset-0 
